...
import { EyeIcon, Plus, X, Trash2, RefreshCw } from 'lucide-react';
...
export default function AdminTechnicians() {
    ...
    const updateLevel = async () => {
        try {
            ...
            showToast(res.data?.message || 'Cập nhật level cho tất cả thợ thành công', 'success');

            // Load lại danh sách nếu level có hiển thị trong danh sách
            await fetchTechnicians(currentPage);
        } catch (err) {
            console.error('Update level error:', err);

            showToast(err.response?.data?.message || 'Không thể cập nhật level cho các thợ', 'error');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="p-6">
            ...
            "
                    >
                        ...
                        {loading ? 'Đang cập nhật...' : 'Update Level'}
                    </button>

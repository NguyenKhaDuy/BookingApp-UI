...
export default function AdminCustomer() {
    ...
    function saveEdit() {
        setCustomers((prev) =>
            prev.map((p) => (p.id_user === selected.id_user ? { ...p, ...form, roleDTOS: [{ role_name: form.role }] } : p))
        );
        setShowEdit(false);
        setSelected(null);
    }
        ...
        <div className="p-6">
            ...
            <div className="overflow-x-auto bg-white shadow rounded-lg border">
                <table className="min-w-full text-sm">
                    ...
                    <tbody>
                        {currentUsers.map((c) => (
                            <tr key={c.id_user} className="border-t hover:bg-gray-50 transition">
                                <td className="p-3">
                                    <img src={c.avatarBase64} className="w-10 h-10 rounded-full" alt="avatar" />
                                </td>
                    ...
                    <div className="bg-white rounded-lg w-full max-w-md p-6 shadow-lg overflow-y-auto max-h-[90vh]">
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (file) {
                                            ...
                                            reader.onload = () => setForm((s) => ({ ...s, avatarBase64: reader.result }));
                                ...
                                />
                                {form.avatarBase64 && (
                                    <img src={form.avatarBase64} alt="Avatar Preview" className="w-20 h-20 rounded-full mt-2" />
                                )}
                            </div>
                        </div>
            ...
            {showDelete && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg w-full max-w-sm p-6 shadow-lg">
                        ...
                        <p className="mt-2 text-sm text-gray-600">Are you sure you want to delete <strong>{selected?.full_name}</strong> ({selected?.id_user})?</p>

                        <div className="mt-4 flex justify-end gap-2">
                            <button onClick={() => { setShowDelete(false); setSelected(null); }} className="px-3 py-2 border rounded">Cancel</button>
                ...
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg w-full max-w-lg p-6 shadow-lg overflow-y-auto max-h-[90vh]">
                        ...
                        <div className="space-y-3 text-sm">
                            <div><strong>ID:</strong> {selected.id_user}</div>
                            <div><strong>Full Name:</strong> {selected.full_name}</div>
                            ...
                            <div>
                                <strong>Role:</strong>{" "}
                                {selected.roleDTOS?.map((r) => r.role_name).join(", ")}
                            </div>

                            <div><strong>Created At:</strong> {selected.created_at}</div>
                            <div><strong>Updated At:</strong> {selected.updated_at}</div>
                        </div>
